...
class CorredoresProvasResultadoSeeder extends Seeder
{
    ...
    public function run()
    {
        $provasCalendario = \CPrado\Models\CorredorProva::orderBy('prova_id')
            ->groupBy('prova_id')
        ->get(['prova_id']);

        $provasCalendario->each(function ($prova) {
            $provaCorrida = \CPrado\Models\ProvaCorrida::select('data_prova')
            ...
            $corredores = \CPrado\Models\CorredorProva::select('id', 'corredor_id')
